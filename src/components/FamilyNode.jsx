...
const FamilyNode = ({ family, members, branches, households, treeData }) => {
  ...
  const renderCustomNodeElement = ({ nodeDatum, toggleNode }) => {
    ...
    switch (nodeDatum.type) {
      case "family":
        return (
          <g onClick={() => setIsExpanded(true)} style={{ cursor: "pointer" }}>
            <rect width="120" height="40" x="-60" y="-20" fill="#ffcccc" stroke="#333" />
      ...
      case "rootAncestor":
        return (
          <g onClick={() => setIsExpanded(false)} style={{ cursor: "pointer" }}>
            <rect width="100" height="40" x="-50" y="-20" fill="#ffd700" stroke="#333" />
            <text x="0" y="0" textAnchor="middle" alignmentBaseline="central">
              {nodeDatum.name}
            </text>
          </g>
      ...
      case "branch":
        return (
          <g onClick={() => toggleBranch(nodeDatum.id)} style={{ cursor: "pointer" }}>
      ...
      case "subhousehold":
        return <SubHouseholdNode nodeDatum={nodeDatum} toggleNode={handleClick} />;
      case "member":
  ...
  const mergedTree = {
    ...baseTree,
    children: [...baseTree.children, ...branchNodes],
  };
